...
namespace KTASimulator
{
    public class KTASimulator : DriverBase.DriverBase
    {
        ...
        private SendResponseDelegate m_XXSRDelegate;
        ...
        /// </summary>
        private List<DriverBase.OrderContext> m_FillList;
        ...
        public KTASimulator( ILog driverLog)
            : base( driverLog)
        {
            Log = driverLog;
            Name = "KTSIM";
            ...
            m_XXSRDelegate = new SendResponseDelegate(this.handleResponse);
        ...
        public Dictionary<string, Market> Markets
        {
            get { return m_Markets; }
            set { m_Markets = value; }
        }
        ...
        protected override void DoStart(string myState)
        {
            try
            {
                ...
                StartTimer();
                ConnectionState = new DriverBase.DriverConnectionStatus[1];
                ConnectionState[0] = new DriverBase.DriverConnectionStatus(StatusConditon.good, StatusConditon.good);
                ConnectionStatusUpdateClients("KTSIM", ConnectionState[0]);
        ...
        /// </summary>
        public override IDriverConnectionStatus[] GetConnectionStatus(IDriverSession[] sessions)
        {
                ...
                try
                {
                    foreach (var set in tsSet)
                    {
                        switch (set.TSType)
                        {
                            ...
                            default:
                                this.driverLog.Error("Unknown TS Request type:" + set.TSType.ToString());
                                break;
                        }
                    }
                }
        ...
        public void CreateMarket(string mnemonic)
        {
            try
            {
                Market market = new Market(mnemonic, this,null, null);
        ...
        public void GetPriceFiles()
        {
            try
            {
                string basePath = AppPath + @"\PriceData";
        ...
        public void AddPriceFile(string mnemonic, string filePath, int interval, bool runRealTime, bool repeatOnEnd, bool playOnSubscribe)
        {
            try
            {
                FilePriceSource pxSrc = new FilePriceSource(this, Log);
        ...
        public void submitOrder(IMessage myMsg)
        {
            ...
            try
            {
                ...
                if (myProd.IsAutoFill)
                {
                    ...
                    sendExecReport(myContext, fill.OrderID, fill.OrderStatus, fill.ExecType, 0M, (int)quantity, 0, myOrdPrice, myOrdPrice, "", "");
                ...
                else
                {
                    ...
                    sendExecReport(myContext, fill.OrderID, fill.OrderStatus, fill.ExecType, 0M, (int)nos.OrderQty, 0,0M, 0M, "", "");
                    ...
                    else
                    {
                        ...
                        if (nos.Mnemonic == "MRK")
                        {
                            ...
                            sendExecReport(myContext, fill.OrderID, fill.OrderStatus, fill.ExecType, (decimal)lastQty, leavesQty, lastQty, myOrdPrice, myOrdPrice, "", "");
                        ...
                        else if (nos.Mnemonic == "ZSELL")
                        {
                            ...
                            sendExecReport(myContext, fill.OrderID, fill.OrderStatus, fill.ExecType, lastQty, leavesQty, lastQty, myOrdPrice, myOrdPrice, "", "");
                        ...
                        else
                        {
                            ...
                            fill.OrderStatus = KaiTrade.Interfaces.OrderStatus.FILLED;
                            fill.ExecType = KaiTrade.Interfaces.ExecType.FILL;
                            sendExecReport(myContext, fill.OrderID, fill.OrderStatus, fill.ExecType, quantity, 0, (int)quantity, myOrdPrice, myOrdPrice, "", "");
                        }
                    }
                }
            }
            catch (Exception myE)
            {
                ...
                if (nos != null)
                {
                    sendExecReport(myMsg as IMessageHeader, nos, OrderStatus.REJECTED, ExecType.REJECTED, myE.ToString(), "OTHER");
                }
            }
        }
        ...
        public void fixedAmountFill(DriverBase.OrderContext myContext, int qtyToFill, decimal? fillPx)
        {
            try
            {
                ...
                if (fillPx.HasValue)
                {
                    fill.LastPx = fillPx.Value;
                }
                else
                {
                    fill.LastPx = myContext.Price;
                }
                fill.AvgPx = 0;
                ...
                if (myContext.LeavesQty > 0)
                {
                    ...
                    sendExecReport(myContext as IMessageHeader, fill);
                }
            }
            catch (Exception myE)
            {
        ...
        public void pullOrder(IMessage msg)
        {
            ...
            try
            {
                ...
                {
                    sendCancelRej(msg as IMessageHeader, cancel, CxlRejReason.UnknownOrder, "a clordid must be specified on a modify order");
                ...
                {
                    sendCancelRej(msg as IMessageHeader, cancel, CxlRejReason.UnknownOrder, "a original clordid must be specified on a modify order");
                ...
                if (myContext == null)
                {
                    sendCancelRej(msg as IMessageHeader, cancel, KaiTrade.Interfaces.CxlRejReason.UnknownOrder, "an order does not exist for the cancel requested");
                ...
                sendExecReport(myContext, fill.OrderID, fill.OrderStatus, fill.ExecType, 0M, (int)myContext.LeavesQty, myContext.CumQty, 0M, 0M, "", "");
        ...
        public override OrderReplaceResult modifyOrder(DriverBase.ModifyRequestData replaceData)
        {
            try
            {
                ...
                sendExecReport(replaceData.OrderContext, fill.OrderID, fill.OrderStatus, fill.ExecType, 0M, replaceData.OrderContext.LeavesQty, replaceData.OrderContext.CumQty, fill.LastPx, fill.AvgPx, "", "");
        ...
        public void modifyOrder(IMessage msg)
        {
            ...
            try
            {
                ...
                {
                    sendCancelReplaceRej(msg as IMessageHeader, mod, CxlRejReason.UnknownOrder, "a clordid must be specified on a modify order");
                    Exception myE = new Exception("a clordid must be specified on a modify order");
                    throw myE;
                }
                ...
                {
                    sendCancelReplaceRej(msg as IMessageHeader, mod, CxlRejReason.UnknownOrder, "a original clordid must be specified on a modify order");
                    Exception myE = new Exception("an original clordid must be specified on a modify order");
                    throw myE;
                }
                ...
                if (myContext == null)
                {
                    sendCancelReplaceRej(msg as IMessageHeader, mod, CxlRejReason.UnknownOrder, "an order does not exist for the modify requested");
        ...
        /// </summary>
        /// <param name="myMsg"></param>
        protected override void DoSend(IMessage myMsg)
        {
            try
            {
                lock (this)
                {
                    ...
                    switch (myMsg.Label)
                    {
                        case KaiTrade.Interfaces.MessageType.NEWORDERSINGLE:
                            submitOrder(myMsg);
                            break;

                        case KaiTrade.Interfaces.MessageType.CANCELORDER:
                            // Pull or Cancel and Order
                            pullOrder(myMsg);
                            break;

                        case KaiTrade.Interfaces.MessageType.MODIFYORDER:
                            modifyOrder(myMsg);
                            break;
                        ...
                        case KaiTrade.Interfaces.MessageType.NOP:
                            base.SendResponse(myMsg as IMessageHeader, KaiTrade.Interfaces.MessageType.NOP, DateTime.Now.ToString());
                            break;
        ...
        protected override void DoStop()
        {
            try
            {
                ...
                ConnectionState[0] = new DriverBase.DriverConnectionStatus(StatusConditon.none, StatusConditon.none);
                ConnectionStatusUpdateClients(Name, ConnectionState[0]);
            }
            catch (Exception myE)
            {
